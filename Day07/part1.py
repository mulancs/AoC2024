...
def recursion(target, running_sum, nums):
	...
	return recursion(target, running_sum + nums[0], nums[1:]) or recursion(target, running_sum * nums[0], nums[1:]) or recursion(target, int(str(running_sum) + str(nums[0])), nums[1:])
